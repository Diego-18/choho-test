<template>
  <div>
    ...
    <div v-else>
      ...
      <div v-else>
        <createBranch v-if="viewCreate" @backList="backList" />
        <editBranch
          v-if="viewUpdate"
...
import createBranch from '@/components/Branch/Branch'
  ...
  setup() {
    ...
    const backList = (value, status) => {
      ...
      if (status === 'update') {
        ...
        textAlert.value = 'Sucursal actualizado con éxito.'
        ...
        showAlert.value = true
        textAlert.value = 'Sucursal registrado con éxito.'
        typeAlert.value = MESSAGE_TYPES.INFO
      }
      getBranches()
    }

    const getBranches = (page = 0, search = '') => {
      loader.value = true
      const dataSearch = {
        search,
        page,
    ...
    const sheetDelete = (value, id) => {
      backendService.value
        ...
        .then((data) => {
          ...
          getBranches(0)
          showAlert.value = true
          textAlert.value = 'Sucursal eliminado con éxito.'
          typeAlert.value = MESSAGE_TYPES.INFO
        })
        .catch((error) => {
          sheetOpenDelete.value = false
