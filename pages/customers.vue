<template>
  <div>
    ...
    <div v-else>
      ...
      <div v-else>
        <createCustomer v-if="viewCreate" @backList="backList" />
        <editCustomer
          v-if="viewUpdate"
  ...
  setup() {
    ...
    const backList = (value, status) => {
      ...
      if (status === 'update') {
        ...
        textAlert.value = 'Cliente actualizado con éxito.'
        ...
        showAlert.value = true
        textAlert.value = 'Cliente registrado con éxito.'
        typeAlert.value = MESSAGE_TYPES.INFO
      }
      getCustomers()
    }

    const getCustomers = (page = 0, search = '') => {
      loader.value = true
      const dataSearch = {
        search,
        page,
    ...
    const sheetDelete = (value, id) => {
      backendService.value
        ...
        .then((data) => {
          ...
          getCustomers(0)
          showAlert.value = true
          textAlert.value = 'Cliente eliminado con éxito.'
          typeAlert.value = MESSAGE_TYPES.INFO
        })
        .catch((error) => {
          sheetOpenDelete.value = false
