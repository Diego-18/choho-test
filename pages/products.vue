<template>
  <div>
    ...
    <div v-else>
      ...
      <div v-else>
        <createProduct v-if="viewCreate" @backList="backList" />
        <editProduct
          v-if="viewUpdate"
  ...
  setup() {
    ...
    const backList = (value, status) => {
      ...
      if (status === 'update') {
        ...
        textAlert.value = 'Producto actualizado con éxito.'
        ...
        showAlert.value = true
        textAlert.value = 'Producto registrado con éxito.'
        typeAlert.value = MESSAGE_TYPES.INFO
      }
      getProducts()
    }

    const getProducts = (page = 0, search = '') => {
      loader.value = true
      const dataSearch = {
        search,
        page,
    ...
    const sheetDelete = (value, id) => {
      backendService.value
        ...
        .then((data) => {
          ...
          getProducts(0)
          showAlert.value = true
          textAlert.value = 'Producto eliminado con éxito.'
          typeAlert.value = MESSAGE_TYPES.INFO
        })
        .catch((error) => {
          sheetOpenDelete.value = false
